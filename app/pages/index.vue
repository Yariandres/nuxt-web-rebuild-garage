...
useSeoMeta({
  ...
  description: 'We fix broken websites like rebuild channels fix wrecked supercars. Teardown → Fix → Test Drive. Book a Rapid Diagnosis today.'
})
</script>
  ...
  <div>
        ...
        <div class="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div>
            <h1 class="text-4xl lg:text-5xl xl:text-6xl font-bold mb-6 leading-tight">
              ...
              <span class="text-amber-500 block">That's how we rescue websites.</span>
            </h1>
            ...
            <p class="text-lg text-zinc-400 mb-8 max-w-xl">
              ...
              goes deeper, I bring in <strong class="text-zinc-200">Yanus</strong>, our DevOps specialist.
          ...
          <div class="hidden lg:block">
            <WorkOrder
              title="Repair Status"
              :items="[
                { label: 'Incoming', value: 'Checkout slow → cart abandonment', status: 'pending' },
    ...
    <section id="symptoms" class="py-20 bg-zinc-900">
      <UContainer>
        <div class="text-center mb-12">
          <h2 class="text-3xl lg:text-4xl font-bold mb-4">What's going on with your site?</h2>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-4 mb-10">
          <NuxtLink
            ...
            class="block"
          >
            <UCard class="h-full hover:border-amber-500/50 transition-colors cursor-pointer group">
              <div class="flex items-start gap-3">
                <UIcon :name="symptom.icon" class="text-amber-500 size-6 shrink-0 mt-1" />
                <div>
                  <h3 class="font-semibold mb-1 group-hover:text-amber-500 transition-colors">
                    {{ symptom.title }}
                  </h3>
      ...
      <UContainer>
        <div class="max-w-3xl mx-auto">
          ...
          <ul class="space-y-4 mb-8">
            ...
            >
              <UIcon name="i-lucide-check-circle-2" class="text-emerald-500 size-6 shrink-0 mt-0.5" />
    ...
    <section class="py-20 bg-zinc-900">
      <UContainer>
        <h2 class="text-3xl lg:text-4xl font-bold mb-12 text-center">
          Our method: <span class="text-amber-500">Teardown → Fix → Test Drive</span>
        </h2>

        <div class="grid md:grid-cols-3 gap-8 mb-12">
          <UCard v-for="(step, index) in methodSteps" :key="step.title">
            <div class="flex items-center gap-3 mb-4">
              <span class="text-3xl font-bold text-amber-500 font-mono">{{ index + 1 }}</span>
    ...
    <section class="py-20">
      <UContainer>
        ...
        <p class="text-xl text-amber-500 mb-12 text-center">You're getting a pit crew.</p>

        <div class="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto mb-10">
          <UCard v-for="member in crew" :key="member.name">
            <div class="flex gap-4">
              <div class="size-16 rounded-full bg-zinc-800 flex items-center justify-center shrink-0">
    ...
    <section class="py-20 bg-zinc-900">
      <UContainer>
        ...
        <div class="grid md:grid-cols-3 gap-8">
          <UCard v-for="service in services" :key="service.name" class="flex flex-col">
            <template #header>
              <div class="flex items-center gap-3">
                <UBadge color="primary" variant="subtle">{{ service.step }}</UBadge>
    ...
    <section class="py-24 bg-zinc-900">
      <UContainer>
        <div class="text-center max-w-2xl mx-auto">
          <h2 class="text-4xl lg:text-5xl font-bold mb-6">Ready for a teardown?</h2>
...
export default {
  data() {
    return {
      symptoms: [
        { title: 'Slow checkout / cart abandonment', detail: 'WooCommerce, scripts, cache, DB, payments', tag: 'checkout', icon: 'i-lucide-shopping-cart' },
        { title: 'Site is slow (Core Web Vitals failing)', detail: 'waterfalls, third-party scripts, assets, caching', tag: 'performance', icon: 'i-lucide-gauge' },
        { title: 'Random errors / broken features', detail: 'plugin conflicts, JS regressions, API issues', tag: 'errors', icon: 'i-lucide-bug' },
        { title: 'Admin dashboard is painfully slow', detail: 'queries, cron, bloat, server limits', tag: 'admin', icon: 'i-lucide-layout-dashboard' },
        { title: 'Deployments / downtime / scaling', detail: 'Yanus territory: infra + monitoring + stability', tag: 'devops', icon: 'i-lucide-server' },
        { title: 'Not sure', detail: "We'll diagnose live on the call", tag: 'unknown', icon: 'i-lucide-help-circle' }
      ],
      deliverables: [
